...
if [[ "$1" == "up" ]]; then
    ...
    if [[ ! -z "$2" ]]; then
        ...
        utils.printer "You should specify the service name (node|mongodb)"
    fi
elif [[ "$1" == "sh" ]]; then
    if [[ ! -z "$2" ]]; then
        utils.printer "Connect to $2 shell"
    ...
    utils.printer "Service names: node | mongodb"
    utils.printer "Params between {} are optional"
    utils.printer "Usage: docker.sh [up|start|restart|stop|rm|sh|bash|logs|ps]"
